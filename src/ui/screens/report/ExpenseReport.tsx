/** ExpenseReport — Real data from expenseStore */
...
export function ExpenseReport() {
  ...
  const byCategory = useMemo(() => {
    const m = new Map<string, number>(); expenses.forEach(e => m.set(e.category, (m.get(e.category)||0)+e.amount));
    const all = Array.from(m.entries()).map(([c, v]) => ({ name: EXPENSE_CATEGORY_LABELS[c as ExpenseCategory]||c, value: v }));
  ...
  const byMonth = useMemo(() => {
    const m = new Map<string, number>(); expenses.forEach(e => { const k = e.date.slice(0,7); m.set(k,(m.get(k)||0)+e.amount); });
