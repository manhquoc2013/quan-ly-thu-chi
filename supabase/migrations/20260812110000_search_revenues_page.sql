-- Search revenues by order_code / notes / customer name in one round-trip.
  ...
  with filtered as (
    ...
    where r.household_id = p_household_id
      ...
      and (
        ...
        or (
          lower(trim(p_search)) = any (array['khách vãng lai', 'khach vang lai', 'vãng lai', 'vang lai'])
...
grant execute on function public.search_revenues_page(
  uuid, text, date, date, text, text, uuid, boolean, integer, integer
) to authenticated;
