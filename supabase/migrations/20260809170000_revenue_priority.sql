-- Priority flag on revenues (đơn ưu tiên)
...
create index if not exists revenues_household_priority_idx
  on public.revenues (household_id, priority desc, priority_at desc nulls last)
  where priority = true;

-- RPC must write priority (payload already sends it; old function ignored the fields)
